// Package ftp implements a parser for the FTP control channel protocol.
package ftp

import (
	"bytes"
	"strconv"
	"strings"

	"github.com/netcap/netcap/internal/protocol"
	"github.com/netcap/netcap/proto"
)

var ftpPrefixes = []string{"220 ", "USER ", "PASS ", "LIST", "RETR ", "STOR ", "QUIT", "CWD ", "PWD", "TYPE ", "PASV", "PORT "}

// Parser decodes FTP control channel commands and responses.
type Parser struct{}

func NewParser() *Parser               { return &Parser{} }
func (p *Parser) Name() string         { return "ftp" }
func (p *Parser) Ports() []uint16      { return []uint16{21} }

// Probe returns a confidence score for FTP traffic.
func (p *Parser) Probe(data []byte, isFromClient bool) int {
	if len(data) < 3 {
		return 0
	}
	s := string(data[:min(len(data), 128)])
	for _, pfx := range []string{"220 ", "USER ", "PASS ", "LIST", "RETR", "STOR"} {
		if strings.HasPrefix(s, pfx) {
			return 90
		}
	}
	return 0
}

// Parse extracts FTP commands and response codes.
func (p *Parser) Parse(data []byte, meta protocol.ConnMeta, isFromClient bool) ([]*proto.ProtocolEvent, error) {
	var events []*proto.ProtocolEvent

	lines := bytes.Split(data, []byte("\r\n"))
	for _, line := range lines {
		if len(line) == 0 {
			continue
		}
		s := string(line)

		if isFromClient {
			ev := proto.NewEvent("ftp", proto.DirectionRequest)
			fillMeta(ev, meta)
			cmd := strings.ToUpper(s)
			if idx := strings.IndexByte(cmd, ' '); idx > 0 {
				cmd = cmd[:idx]
			}
			ev.Metadata = map[string]string{"command": cmd, "line": s}
			events = append(events, ev)
		} else {
			if len(s) >= 3 {
				if code, err := strconv.Atoi(s[:3]); err == nil {
					ev := proto.NewEvent("ftp", proto.DirectionResponse)
					fillMeta(ev, meta)
					msg := ""
					if len(s) > 4 {
						msg = s[4:]
					}
					ev.Metadata = map[string]string{
						"code":    strconv.Itoa(code),
						"message": msg,
					}
					events = append(events, ev)
				}
			}
		}
	}
	return events, nil
}

func fillMeta(ev *proto.ProtocolEvent, meta protocol.ConnMeta) {
	ev.SrcIP = meta.SrcIP
	ev.DstIP = meta.DstIP
	ev.SrcPort = uint32(meta.SrcPort)
	ev.DstPort = uint32(meta.DstPort)
}

func min(a, b int) int {
	if a < b {
		return a
	}
	return b
}

var _ protocol.Parser = (*Parser)(nil)
